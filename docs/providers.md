...
How Spanner integrates each agent runtime. The provider seam lives under `packages/server/src/server/agent/providers/`. Each provider implements the `AgentClient` interface declared in `agent-sdk-types.ts`.
...
- **Generic ACP** — any user-defined `extends: "acp"` provider via `~/.spanner/config.json`
...
Stored per-session at `~/.spanner/agents/<sanitized-cwd>/<session-id>.json`. Lets clients resume across daemon restarts without re-supplying config.
...
**Spanner never manages API keys.** Each provider CLI reads its own creds (`~/.claude`, `~/.codex/auth.json`, etc.). The daemon inherits the user's shell env via `createProviderEnvSpec`.
