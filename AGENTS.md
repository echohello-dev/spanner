...
Spanner ships agent skills in `skills/<name>/SKILL.md` (Paseo convention). Skills are installed via `npx skills add echohello-dev/spanner`. The skill content teaches an external AI agent how to drive the Spanner daemon via its MCP server tools (planned post-MVP).
