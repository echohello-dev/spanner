# AGENTS.md — Operating manual for AI coding agents working on Spanner

## What this project is

Spanner is a local-first, multi-surface coding-agent workbench. A single supervised daemon (`@echohello/server`) owns agent lifecycle, worktree management, and remote access. Three renderers consume the daemon over WebSocket: `@echohello/web` (desktop + PWA, Vite + React), `@echohello/desktop` (Electron shell), `@echohello/app` (Expo + React Native, mobile).
...
Spanner ships agent skills in two locations, both using the Paseo/Spanner `SKILL.md` convention:

| Location                         | Audience                                                                                                              | Installed via                                  | Tracked?                                      |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | --------------------------------------------- |
| `skills/<name>/SKILL.md` (root)  | **External** users — published for the world to install                                                               | `npx skills add echohello-dev/spanner`         | ✅ committed                                  |
...
The skill content teaches an external AI agent how to drive the Spanner daemon via its MCP server tools (planned post-MVP).
