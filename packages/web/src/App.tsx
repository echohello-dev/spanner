...
import { SpannerClient, type SpannerClient as SpannerClientType } from "@echohello/client";
...
export function App() {
  const [client, setClient] = useState<SpannerClientType | null>(null);
  const [helloAck, setHelloAck] = useState<Awaited<
    ReturnType<SpannerClientType["connect"]>
  > | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const c = new SpannerClient({
      endpoint: DAEMON_WS,
      clientId: CLIENT_ID,
      clientType: "web",
