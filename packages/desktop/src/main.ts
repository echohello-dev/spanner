...
async function ensureDaemon(): Promise<DaemonHandle> {
  ...
  log.info("starting embedded Spanner daemon");
  daemonHandle = await startDaemon({
    spannerHome: path.join(app.getPath("userData"), "spanner-home"),
  });
  return daemonHandle;
}
...
ipcMain.handle("spanner:window.setTitle", (_event, title: string) => {
