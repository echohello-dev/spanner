...
const SAMPLE_OFFER = {
  ...
  relay: { endpoint: "relay.spanner.run:443", useTls: true },
...
describe("connection-offer", () => {
  it("round-trips through a fragment URL", () => {
    const url = encodeOfferToFragmentUrl({
      offer: SAMPLE_OFFER,
      appBaseUrl: "https://app.spanner.run",
    });
    expect(url).toMatch(/^https:\/\/app\.spanner\.run\/#offer=/);
  ...
  it("returns null when there is no offer fragment", () => {
    expect(parseConnectionOfferFromUrl("https://app.spanner.run/")).toBeNull();
  });

  it("throws on a malformed payload in the fragment", () => {
    expect(() => parseConnectionOfferFromUrl("https://app.spanner.run/#offer=not-json")).toThrow();
  });
