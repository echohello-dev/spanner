...
/**
 * Provider manifest — declares which agent runtimes Spanner ships with
 * and their default modes. Users may override or extend via
 * `~/.spanner/config.json` (`agents.providers` map).
 */
...
export const ProviderDefinitionSchema = z.object({
  ...
  /** Where Spanner resolves the binary on disk. */
  command: z.array(z.string()).optional(),
  /** Env vars Spanner overlays on the provider CLI (provider reads its own keys). */
 ...
 * `~/.spanner/config.json` under `agents.providers`.
 *
 ...
 */
export const NO_MODE = Symbol.for("spanner.noMode");
