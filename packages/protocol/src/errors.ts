...
export const SpannerErrorPayloadSchema = z.object({
  code: ErrorCodeSchema,
  message: z.string(),
  details: z.unknown().optional(),
  stack: z.string().optional(),
});
export type SpannerErrorPayload = z.infer<typeof SpannerErrorPayloadSchema>;
...
export class SpannerError extends Error {
  readonly code: ErrorCode;
  readonly details: unknown;
  constructor(args: SpannerErrorOptions) {
    super(args.message, args.cause === undefined ? undefined : { cause: args.cause });
    this.name = "SpannerError";
    this.code = args.code;
    this.details = args.details;
  }

  toPayload(): SpannerErrorPayload {
    return SpannerErrorPayloadSchema.parse({
      code: this.code,
...
function makeLayerError(layer: "DaemonError" | "AgentError" | "ProviderError") {
  return class extends SpannerError {
...
export class DaemonError extends SpannerError {
  constructor(args: LayerErrorOptions) {
...
export class AgentError extends SpannerError {
  constructor(args: LayerErrorOptions) {
    super({ ...args, code: args.code ?? "internal" });
...
export class ProviderError extends SpannerError {
  constructor(args: LayerErrorOptions) {
