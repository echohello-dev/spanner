...
import type { SpannerClientListener, SpannerClientEventMap, Unsubscribe } from "./types.js";

/**
 * Low-level WebSocket driver for the Spanner daemon protocol.
 *
 ...
 *
 * Use `SpannerClient` for the high-level facade with typed RPC + subscribe APIs.
 */
export class WsDriver {
  ...
  #listeners = new Map<keyof SpannerClientEventMap, Set<(payload: unknown) => void>>();
  ...
  send(envelope: Envelope): void {
    if (!this.#ws || this.#ws.readyState !== WebSocket.OPEN) {
      throw new Error("Spanner WS driver is not connected");
    }
  ...
  on<E extends keyof SpannerClientEventMap>(
    event: E,
    listener: SpannerClientListener<E>,
  ): Unsubscribe {
  ...
  #emit<E extends keyof SpannerClientEventMap>(event: E, payload: SpannerClientEventMap[E]): void {
