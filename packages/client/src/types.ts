...
export interface SpannerClientOptions {
  /** Local or relay daemon endpoint. e.g. `ws://127.0.0.1:6767` or `wss://relay.spanner.run/ws`. */
  endpoint: string;
  clientId: string;
...
export interface SpannerClientEventMap {
...
export type SpannerClientListener<E extends keyof SpannerClientEventMap> = (
  payload: SpannerClientEventMap[E],
) => void;
