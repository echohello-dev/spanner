...
export function handleHello(args: {
  ...
  const ack = HelloAckMessageSchema.parse({
    ...
    protocolVersion: SPANNER_PROTOCOL_VERSION,
    capabilities: {
...
export function loadOrCreateIdentity(spannerHome: string): DaemonIdentity {
  const serverId = getOrCreateServerId(spannerHome);
  const kp = loadOrCreateDaemonKeyPair(spannerHome);
  return {
    serverId,
